...
public final class ContinuableRecordOutput implements LittleEndianOutput {
   ...
   private void writeCharacterData(String text, boolean is16bitEncoded) {
      ...
      if(is16bitEncoded) {
         while(true) {
            for(nWritableChars = Math.min(nChars - i, this._ulrOutput.getAvailableSpace() / 2); nWritableChars > 0; --nWritableChars) {
         ...
         while(true) {
            for(nWritableChars = Math.min(nChars - i, this._ulrOutput.getAvailableSpace() / 1); nWritableChars > 0; --nWritableChars) {
