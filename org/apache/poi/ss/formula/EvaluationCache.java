...
final class EvaluationCache {
   ...
   public void notifyDeleteCell(int bookIndex, int sheetIndex, EvaluationCell cell) {
      if(cell.getCellTypeEnum() == CellType.FORMULA) {
         FormulaCellCacheEntry loc = this._formulaCellCache.remove(cell);
         if(loc != null) {
            loc.setSensitiveInputCells((CellCacheEntry[])null);
            loc.recurseClearCachedFormulaResults(this._evaluationListener);
         }
      } else {
         PlainCellCache.Loc loc1 = new PlainCellCache.Loc(bookIndex, sheetIndex, cell.getRowIndex(), cell.getColumnIndex());
         PlainValueCellCacheEntry pcce = this._plainCellCache.get(loc1);
         if(pcce != null) {
            pcce.recurseClearCachedFormulaResults(this._evaluationListener);
         }
      }
