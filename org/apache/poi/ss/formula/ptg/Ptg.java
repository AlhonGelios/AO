...
public abstract class Ptg {
   ...
   public static Ptg[] readTokens(int size, LittleEndianInput in) {
      ...
      while(pos < size) {
         Ptg result = createPtg(in);
         if(result instanceof ArrayPtg.Initial) {
            hasArrayPtgs = true;
         }
      ...
      if(pos != size) {
         ...
         Ptg[] var7 = toPtgArray(temp);

         for(int i = 0; i < var7.length; ++i) {
            if(var7[i] instanceof ArrayPtg.Initial) {
               var7[i] = ((ArrayPtg.Initial)var7[i]).finishReading(in);
            }
         }
   ...
   public static int serializePtgs(Ptg[] ptgs, byte[] array, int offset) {
      ...
      Ptg[] i$ = ptgs;
      int arrayPtg = ptgs.length;

      for(int p = 0; p < arrayPtg; ++p) {
         Ptg ptg = i$[p];
         ptg.write(out);
         if(ptg instanceof ArrayPtg) {
            if(arrayPtgs == null) {
      ...
      if(arrayPtgs != null) {
         Iterator var9 = arrayPtgs.iterator();

         while(var9.hasNext()) {
            Ptg var10 = (Ptg)var9.next();
            ArrayPtg var11 = (ArrayPtg)var10;
            var11.writeTokenValueBytes(out);
         }
      }
   ...
   private static boolean isDeletedCellRef(Ptg ptg) {
      return ptg == ErrPtg.REF_INVALID?true:(ptg instanceof DeletedArea3DPtg?true:(ptg instanceof DeletedRef3DPtg?true:(ptg instanceof AreaErrPtg?true:ptg instanceof RefErrorPtg)));
   }
