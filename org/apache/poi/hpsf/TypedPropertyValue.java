...
class TypedPropertyValue {
   ...
   int read(byte[] data, int startOffset) {
      this._type = LittleEndian.getShort(data, startOffset);
      int offset = startOffset + 2;
      short padding = LittleEndian.getShort(data, offset);
      offset += 2;
      if(padding != 0) {
         logger.log(5, new Object[]{"TypedPropertyValue padding at offset " + offset + " MUST be 0, but it\'s value is " + padding});
      }

      offset += this.readValue(data, offset);
   ...
   int readValuePadded(byte[] data, int offset) {
      int nonPadded = this.readValue(data, offset);
