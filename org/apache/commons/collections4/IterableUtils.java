...
public class IterableUtils {
   ...
   public static int frequency(Iterable iterable, Object obj) {
      return iterable instanceof Set?(((Set)iterable).contains(obj)?1:0):(iterable instanceof Bag?((Bag)iterable).getCount(obj):size(filteredIterable(emptyIfNull(iterable), EqualPredicate.equalPredicate(obj))));
   }
   ...
   public static List partition(Iterable iterable, Factory partitionFactory, Predicate ... predicates) {
      if(iterable == null) {
         Iterable var12 = emptyIterable();
         return partition(var12, partitionFactory, predicates);
      } else if(predicates == null) {
         throw new NullPointerException("Predicates must not be null.");
      } else {
         Predicate[] numberOfPredicates = predicates;
         int numberOfPartitions = predicates.length;

         for(int partitions = 0; partitions < numberOfPartitions; ++partitions) {
            Predicate i$ = numberOfPredicates[partitions];
            if(i$ == null) {
               throw new NullPointerException("Predicate must not be null.");
            }
         }
         ...
         if(predicates.length < 1) {
            Collection var11 = (Collection)partitionFactory.create();
            CollectionUtils.addAll(var11, iterable);
            return Collections.singletonList(var11);
         } else {
            int var10 = predicates.length;
            numberOfPartitions = var10 + 1;
            ArrayList var13 = new ArrayList(numberOfPartitions);

            for(int var14 = 0; var14 < numberOfPartitions; ++var14) {
               var13.add(partitionFactory.create());
            }
            ...
            while(var15.hasNext()) {
               ...
               while(true) {
                  if(i < var10) {
                     ...
                     ((Collection)var13.get(i)).add(element);
                     elementAssigned = true;
                  }

                  if(!elementAssigned) {
                     ((Collection)var13.get(var10)).add(element);
                  }
                  break;
               }
            }
