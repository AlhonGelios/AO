...
public class CompositeMap extends AbstractIterableMap implements Serializable {
   ...
   public CompositeMap() {
      this(new Map[0], (CompositeMap.MapMutator)null);
   }

   public CompositeMap(Map one, Map two) {
      this(new Map[]{one, two}, (CompositeMap.MapMutator)null);
   }
   ...
   public CompositeMap(Map ... composite) {
      this(composite, (CompositeMap.MapMutator)null);
   }
   ...
   public synchronized void addComposited(Map map) throws IllegalArgumentException {
      for(int temp = this.composite.length - 1; temp >= 0; --temp) {
         Collection intersect = CollectionUtils.intersection(this.composite[temp].keySet(), map.keySet());
         if(intersect.size() != 0) {
            if(this.mutator == null) {
               throw new IllegalArgumentException("Key collision adding Map to CompositeMap");
            }

            this.mutator.resolveCollision(this, this.composite[temp], map, intersect);
         }
      }

      Map[] var4 = new Map[this.composite.length + 1];
      System.arraycopy(this.composite, 0, var4, 0, this.composite.length);
      var4[var4.length - 1] = map;
      this.composite = var4;
   }
