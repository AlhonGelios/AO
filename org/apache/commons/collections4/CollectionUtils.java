...
public class CollectionUtils {
   ...
   public static void transform(Collection collection, Transformer transformer) {
      if(collection != null && transformer != null) {
         if(collection instanceof List) {
            List resultCollection = (List)collection;
            ListIterator it = resultCollection.listIterator();

            while(it.hasNext()) {
               it.set(transformer.transform(it.next()));
            }
         } else {
            Collection resultCollection1 = collect((Iterable)collection, transformer);
            collection.clear();
            collection.addAll(resultCollection1);
         }
      }
   ...
   public static Object get(Object object, int index) {
      ...
      if(index < 0) {
         ...
         Map ex3 = (Map)object;
         Iterator iterator = ex3.entrySet().iterator();
         return IteratorUtils.get(iterator, index);
         ...
         if(object instanceof Iterator) {
            ex1 = (Iterator)object;
            return IteratorUtils.get(ex1, index);
         } else if(object instanceof Iterable) {
            Iterable ex2 = (Iterable)object;
            return IterableUtils.get(ex2, index);
         } else if(object instanceof Collection) {
            ex1 = ((Collection)object).iterator();
            return IteratorUtils.get(ex1, index);
         } else if(object instanceof Enumeration) {
            Enumeration ex = (Enumeration)object;
            return EnumerationUtils.get(ex, index);
         } else if(object == null) {
   ...
   public static boolean isFull(Collection coll) {
      if(coll == null) {
         ...
         try {
            BoundedCollection ex = UnmodifiableBoundedCollection.unmodifiableBoundedCollection(coll);
            return ex.isFull();
         } catch (IllegalArgumentException var2) {
            return false;
         }
   ...
   public static List collate(Iterable a, Iterable b, Comparator c, boolean includeDuplicates) {
      if(a != null && b != null) {
         if(c == null) {
            ...
            CollatingIterator iterator = new CollatingIterator(c, a.iterator(), b.iterator());
   ...
   public static Collection retainAll(Iterable collection, Iterable retain, final Equator equator) {
      ...
      Set retainSet = (Set)collect(retain, transformer, new HashSet());
      ArrayList list = new ArrayList();
   ...
   public static Collection removeAll(Iterable collection, Iterable remove, final Equator equator) {
      ...
      Set removeSet = (Set)collect(remove, transformer, new HashSet());
      ArrayList list = new ArrayList();
      Iterator i$ = collection.iterator();
