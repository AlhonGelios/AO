...
public final class SOAPArrayType {
   ...
   private static int[] internalParseCommaIntString(String csl) {
      ArrayList dimStrings = new ArrayList();
      int i = 0;

      while(true) {
         int result = csl.indexOf(44, i);
         if(result < 0) {
            dimStrings.add(csl.substring(i));
            int[] var8 = new int[dimStrings.size()];
            i = 0;
            ...
            for(Iterator it = dimStrings.iterator(); it.hasNext(); ++i) {
               ...
               if(!dimString.equals("*") && !dimString.equals("")) {
                  try {
                     var8[i] = Integer.parseInt(dimString);
         ...
         dimStrings.add(csl.substring(i, result));
         i = result + 1;
      }
   }
   ...
   private void initDimensions(String s, int firstbrace) {
      ...
      for(int trailer = firstbrace; trailer >= 0; trailer = s.indexOf(91, lastbrace)) {
         lastbrace = s.indexOf(93, trailer);
         if(lastbrace < 0) {
            throw new XmlValueOutOfRangeException();
         }

         braces.add(s.substring(trailer + 1, lastbrace));
      }

      String var11 = s.substring(lastbrace + 1);
      if(!XmlWhitespace.isAllSpace(var11)) {
         throw new XmlValueOutOfRangeException();
      } else {
   ...
   public static SOAPArrayType newSoap12Array(QName itemType, String arraySize) {
      ...
      for(int sot = 0; sot < dimStrings.length; ++sot) {
         String dimString = dimStrings[sot];
         if(sot == 0 && dimString.equals("*")) {
            dimensions[sot] = -1;
         } else {
            try {
               dimensions[sot] = Integer.parseInt(dimStrings[sot]);
            } catch (Exception var8) {
               throw new XmlValueOutOfRangeException();
            }
         }
      ...
      SOAPArrayType var9 = new SOAPArrayType();
      var9._ranks = ranks;
      var9._type = itemType;
      var9._dimensions = dimensions;
      return var9;
   }
