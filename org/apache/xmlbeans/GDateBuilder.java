...
public final class GDateBuilder implements GDateSpecification, Serializable {
      ...
      if(date.hasYear() && date.getYear() == 0) {
         ...
         if(date.getDay() >= 1 && date.getDay() <= 31) {
            if(date.getDay() <= 28 || !date.hasMonth()) {
               return date.hasTime() && (date.getHour() < 0 || date.getHour() > 23 || date.getMinute() < 0 || date.getMinute() > 59 || date.getSecond() < 0 || date.getSecond() > 59 || date.getFraction().signum() < 0 || date.getFraction().compareTo(GDate._one) >= 0) && (date.getHour() != 24 || date.getMinute() != 0 || date.getSecond() != 0 || date.getFraction().compareTo(GDate._zero) != 0)?false:!date.hasTimeZone() || date.getTimeZoneSign() == 0 && date.getTimeZoneHour() == 0 && date.getTimeZoneMinute() == 0 || (date.getTimeZoneSign() == -1 || date.getTimeZoneSign() == 1) && date.getTimeZoneHour() >= 0 && date.getTimeZoneMinute() >= 0 && (date.getTimeZoneHour() == 14 && date.getTimeZoneMinute() == 0 || date.getTimeZoneHour() < 14 && date.getTimeZoneMinute() < 60);
            }
   ...
   public void normalize() {
      ...
      if(this.hasTime() && this._fs != null && this._fs.scale() > 0) {
         if(this._fs.signum() == 0) {
            ...
            int lastzero;
            for(lastzero = str.length(); lastzero > 0 && str.charAt(lastzero - 1) == 48; --lastzero) {
               ;
            }
   ...
   public void normalizeToTimeZone(int tzSign, int tzHour, int tzMinute) {
      ...
      } else if(this.hasTimeZone() && this.hasTime()) {
   ...
   private static int _maxDayInMonthFor(int year, int month) {
      return month != 4 && month != 6 && month != 9 && month != 11?(month == 2?(_isLeapYear(year)?29:28):31):30;
   }

   private static int _maxDayInMonth(int month) {
      return month != 4 && month != 6 && month != 9 && month != 11?(month == 2?29:31):30;
   }
   ...
   static final int compareGDate(GDateSpecification tdate, GDateSpecification datespec) {
      ...
      if((bitdiff & 31) == 0) {
         if(((GDateSpecification)tdate).hasTimeZone() && (((GDateSpecification)datespec).getTimeZoneHour() != ((GDateSpecification)tdate).getTimeZoneHour() || ((GDateSpecification)datespec).getTimeZoneMinute() != ((GDateSpecification)tdate).getTimeZoneMinute() || ((GDateSpecification)datespec).getTimeZoneSign() != ((GDateSpecification)tdate).getTimeZoneSign())) {
            ...
            pdate1 = ((GDateSpecification)tdate).getFlags() & 14;
            if(pdate1 != 0 && pdate1 != 14 || !((GDateSpecification)tdate).hasTime()) {
         ...
         pdate1 = compareGDate((GDateSpecification)datespec, (GDateSpecification)tdate);
         return pdate1 == 2?2:-pdate1;
      } else {
   ...
   public String canonicalString() {
      ...
      if(!needNormalize && this.getFraction() != null && this.getFraction().scale() > 0) {
         BigInteger cdate = this.getFraction().unscaledValue();
         needNormalize = cdate.mod(TEN).signum() == 0;
      }

      if(!needNormalize) {
         return this.toString();
      } else {
         GDateBuilder cdate1 = new GDateBuilder(this);
         cdate1.normalize();
         return cdate1.toString();
      }
   }
