...
public class XMLNameHelper {
   ...
   public static String pretty(XMLName name) {
      return name == null?"null":(name.getNamespaceUri() != null && name.getNamespaceUri().length() != 0?name.getLocalName() + "@" + name.getNamespaceUri():name.getLocalName());
   }
   ...
   public static String hexsafe(String s) {
      ...
      for(int i = 0; i < s.length(); ++i) {
         ...
         if(isSafe(ch)) {
            ...
            try {
               byte[] var7 = s.substring(i, i + 1).getBytes("UTF-8");

               for(int uee = 0; uee < var7.length; ++uee) {
                  result.append('_');
                  result.append(hexdigits[var7[uee] >> 4 & 15]);
                  result.append(hexdigits[var7[uee] & 15]);
               }
