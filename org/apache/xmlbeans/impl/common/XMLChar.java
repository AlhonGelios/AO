...
public class XMLChar {
   ...
   static {
      int[] charRange = new int[]{9, 10, 13, 13, 32, '\ud7ff', '\ue000', '\ufffd'};
      ...
      for(i = 0; i < charRange.length; i += 2) {
         for(j = charRange[i]; j <= charRange[i + 1]; ++j) {
            CHARS[j] = (byte)(CHARS[j] | 33);
         }
      }

      for(i = 0; i < specialChar.length; ++i) {
         CHARS[specialChar[i]] &= -33;
      }

      for(i = 0; i < spaceChar.length; ++i) {
         CHARS[spaceChar[i]] = (byte)(CHARS[spaceChar[i]] | 2);
      }

      for(i = 0; i < nameStartChar.length; ++i) {
         CHARS[nameStartChar[i]] = (byte)(CHARS[nameStartChar[i]] | 204);
      }

      for(i = 0; i < letterRange.length; i += 2) {
         for(j = letterRange[i]; j <= letterRange[i + 1]; ++j) {
            CHARS[j] = (byte)(CHARS[j] | 204);
         }
      }

      for(i = 0; i < letterChar.length; ++i) {
         CHARS[letterChar[i]] = (byte)(CHARS[letterChar[i]] | 204);
      }

      for(i = 0; i < nameChar.length; ++i) {
         CHARS[nameChar[i]] = (byte)(CHARS[nameChar[i]] | 136);
      }

      for(i = 0; i < digitRange.length; i += 2) {
         for(j = digitRange[i]; j <= digitRange[i + 1]; ++j) {
            CHARS[j] = (byte)(CHARS[j] | 136);
         }
      }
      ...
      for(i = 0; i < combiningCharChar.length; ++i) {
         CHARS[combiningCharChar[i]] = (byte)(CHARS[combiningCharChar[i]] | 136);
      }
