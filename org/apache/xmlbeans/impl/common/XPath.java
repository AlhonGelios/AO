...
public class XPath {
   ...
   private final XPath.Selector _selector;
   ...
   public static XPath compileXPath(String xpath, String currentNodeVar, Map namespaces) throws XPath.XPathCompileException {
      return (new XPath.CompilationContext(namespaces, currentNodeVar)).compile(xpath);
   }

   private XPath(XPath.Selector selector, boolean sawDeepDot) {
   ...
   static XPath.Selector access$000(XPath x0) {
      return x0._selector;
   }

   XPath(XPath.Selector x0, boolean x1, XPath.NamelessClass2074112064 x2) {
      this(x0, x1);
   }
   ...
   private static class CompilationContext {
      ...
      private int _line;
      private int _column;
      static final boolean $assertionsDisabled = !XPath.class.desiredAssertionStatus();
         ...
         if(this._offset < this._expr.length()) {
            ...
            if(ch == 13 || ch == 10) {
               ...
               if(this._offset + 1 < this._expr.length()) {
                  ...
                  if((nextCh == 13 || nextCh == 10) && ch != nextCh) {
                     ++this._offset;
                  }
               }
            }
      ...
      private boolean tokenize(String s1, String s2) {
         if(!$assertionsDisabled && s1.length() <= 0) {
            throw new AssertionError();
         } else if(!$assertionsDisabled && s2.length() <= 0) {
            throw new AssertionError();
         } else {
            int offset;
            for(offset = 0; this.isWhitespace(offset); ++offset) {
      ...
      private boolean tokenize(String s1, String s2, String s3) {
         if(!$assertionsDisabled && s1.length() <= 0) {
            ...
            throw new AssertionError();
         } else if(!$assertionsDisabled && s3.length() <= 0) {
            throw new AssertionError();
         } else {
            int offset;
            for(offset = 0; this.isWhitespace(offset); ++offset) {
      ...
      private boolean tokenize(String s1, String s2, String s3, String s4) {
         if(!$assertionsDisabled && s1.length() <= 0) {
            ...
            throw new AssertionError();
         } else if(!$assertionsDisabled && s4.length() <= 0) {
            throw new AssertionError();
         } else {
            int offset;
            for(offset = 0; this.isWhitespace(offset); ++offset) {
      ...
      private XPath.Step addStep(boolean deep, boolean attr, QName name, XPath.Step steps) {
         XPath.Step step = new XPath.Step(deep, attr, name);
         if(steps == null) {
            return step;
         } else {
            XPath.Step s;
      ...
      private XPath.Step tokenizeSteps() throws XPath.XPathCompileException {
         if(this.tokenize("/")) {
            ...
            if(!this.tokenize("$", this._currentNodeVar, "//") && !this.tokenize(".", "//")) {
               if(!this.tokenize("$", this._currentNodeVar, "/") && !this.tokenize(".", "/")) {
                  if(this.tokenize("$", this._currentNodeVar) || this.tokenize(".")) {
                     return this.addStep(false, false, (QName)null, (XPath.Step)null);
                  }
      ...
      private void computeBacktrack(XPath.Step steps) throws XPath.XPathCompileException {
         XPath.Step t;
         for(XPath.Step s = steps; s != null; s = t) {
            for(t = s._next; t != null && !t._deep; t = t._next) {
               ;
            }

            if(!s._deep) {
               for(XPath.Step n = s; n != t; n = n._next) {
                  n._hasBacktrack = true;
               }
            } else {
               int var11 = 0;
               ...
               QName[] pattern = new QName[var11 + 1];
               int[] kmp = new int[var11 + 1];
               XPath.Step v = s;
               ...
               pattern[var11] = this.getAnyQName();
               i = 0;
               int j = kmp[0] = -1;
      ...
      private void tokenizePath(ArrayList paths) throws XPath.XPathCompileException {
         ...
         XPath.Step steps = this.tokenizeSteps();
         ...
         if(this._lastDeepDot) {
            ...
            for(XPath.Step t = steps; t != null; t = t._next) {
      ...
      private XPath.Selector tokenizeSelector() throws XPath.XPathCompileException {
         ...
         return new XPath.Selector((XPath.Step[])((XPath.Step[])paths.toArray(new XPath.Step[0])));
      }
      ...
      private XPath tokenizeXPath() throws XPath.XPathCompileException {
         while(true) {
            ...
            if(this.tokenize("declare", "namespace")) {
               ...
               selector = this.tokenizeNCName();
               if(!this.tokenize("=")) {
                  throw this.newError("Expected \'=\'");
               }

               String uri = this.tokenizeQuotedUri();
               if(this._namespaces.containsKey(selector)) {
                  throw this.newError("Redefinition of namespace prefix: " + selector);
               }

               this._namespaces.put(selector, uri);
               if(this._externalNamespaces.containsKey(selector)) {
                  throw this.newError("Redefinition of namespace prefix: " + selector);
               }

               this._externalNamespaces.put(selector, uri);
               if(!this.tokenize(";")) {
                  ;
               }
               ...
               if(!this.tokenize("declare", "default", "element", "namespace")) {
                  ...
                  XPath.Selector selector1 = this.tokenizeSelector();
                  this.parseWhitespace();
                  if(this.currChar() != -1) {
                     throw this.newError("Unexpected char \'" + (char)this.currChar() + "\'");
                  }

                  return new XPath(selector1, this._sawDeepDot, (XPath.NamelessClass2074112064)null);
               }

               selector = this.tokenizeQuotedUri();
               if(this._namespaces.containsKey("")) {
                  throw this.newError("Redefinition of default element namespace");
               }

               this._namespaces.put("", selector);
               if(this._externalNamespaces.containsKey("$xmlbeans!default_uri")) {
                  throw this.newError("Redefinition of default element namespace : ");
               }

               this._externalNamespaces.put("$xmlbeans!default_uri", selector);
               if(!this.tokenize(";")) {
   ...
   static class NamelessClass2074112064 {
   }
   ...
   public static class ExecutionContext {
      ...
      private XPath.ExecutionContext.PathContext[] _paths;
      static final boolean $assertionsDisabled = !XPath.class.desiredAssertionStatus();
      ...
      public final void init(XPath xpath) {
         ...
         if(this._xpath != xpath) {
            this._xpath = xpath;
            this._paths = new XPath.ExecutionContext.PathContext[xpath._selector._paths.length];

            for(i = 0; i < this._paths.length; ++i) {
               this._paths[i] = new XPath.ExecutionContext.PathContext();
            }
         }
      ...
      static ArrayList access$100(XPath.ExecutionContext x0) {
         return x0._stack;
      }
      ...
      private final class PathContext {
         ...
         private void backtrack() {
            if(!$assertionsDisabled && this._curr == null) {
               throw new AssertionError();
            } else if(this._curr._hasBacktrack) {
               this._curr = this._curr._backtrack;
            } else if(!$assertionsDisabled && this._curr._deep) {
               throw new AssertionError();
            } else {
               ...
               while(!this._curr._deep) {
                  ...
                  for(XPath.Step s = this._curr; !s._deep; s = s._prev) {
         ...
         int start() {
            if(!$assertionsDisabled && this._curr == null) {
               throw new AssertionError();
            } else if(!$assertionsDisabled && this._curr._prev != null) {
               throw new AssertionError();
            } else if(this._curr._name != null) {
         ...
         int element(QName name) {
            ...
            if(this._curr == null) {
               return 0;
            } else if(!$assertionsDisabled && this._curr._name == null) {
               throw new AssertionError();
            } else if(!this._curr._attr && this._curr.match(name)) {
         ...
         void end() {
            this._curr = (XPath.Step)this._prev.remove(this._prev.size() - 1);
         }
   ...
   private static final class Step {
      ...
      boolean match(QName name) {
         ...
         if(localLength == 0) {
            ...
            return uri.length() != nameUri.length()?false:local.equals(nameLocal) && uri.equals(nameUri);
         }
      }
   }
}
