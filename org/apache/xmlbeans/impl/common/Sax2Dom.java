...
public class Sax2Dom extends DefaultHandler implements ContentHandler, LexicalHandler {
   ...
   public void startElement(String namespace, String localName, String qName, Attributes attrs) {
      ...
      if(this._namespaceDecls != null) {
         ...
         for(last = 0; last < nattrs; ++last) {
            String prefix = (String)this._namespaceDecls.elementAt(last++);
            if(prefix != null && !prefix.equals("")) {
               tmp.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:" + prefix, (String)this._namespaceDecls.elementAt(last));
            } else {
               tmp.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns", (String)this._namespaceDecls.elementAt(last));
            }
         }
      ...
      for(last = 0; last < nattrs; ++last) {
         if(attrs.getLocalName(last) == null) {
            tmp.setAttribute(attrs.getQName(last), attrs.getValue(last));
         } else {
            tmp.setAttributeNS(attrs.getURI(last), attrs.getQName(last), attrs.getValue(last));
         }
      }

      Node var9 = (Node)this._nodeStk.peek();
      var9.appendChild(tmp);
