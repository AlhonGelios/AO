...
import org.apache.xmlbeans.impl.piccolo.xml.XMLReaderReader;
...
public class EntityManager {
   ...
   public boolean putInternal(String name, String value, int type) {
      if(this.entityMaps[type].get(name) == null) {
         this.entityMaps[type].put(name, new EntityManager.Entry(value));
         return true;
      } else {
         return false;
      }
   ...
   public boolean putExternal(Entity context, String name, String pubID, String sysID, int type) throws MalformedURLException {
      if(this.entityMaps[type].get(name) == null) {
         ...
         EntityManager.Entry e = new EntityManager.Entry(pubID, sysID);
   ...
   public boolean putUnparsed(Entity context, String name, String pubID, String sysID, String ndata, int type) throws MalformedURLException {
      if(this.entityMaps[type].get(name) == null) {
         this.entityMaps[type].put(name, new EntityManager.Entry(pubID, sysID, ndata));
         return true;
      } else {
         return false;
      }
   ...
   public Entity getByName(String name, int type) {
      return (EntityManager.Entry)this.entityMaps[type].get(name);
   }
   ...
   public Entity getByID(Entity context, String pubID, String sysID) throws MalformedURLException {
      ...
      if(result != null) {
         ...
         EntityManager.Entry result1 = new EntityManager.Entry(pubID, sysID);
         if(pubID != null && pubID.length() > 0) {
            this.entitiesByURI.put(pubID, result1);
         }

         this.entitiesByURI.put(sysID, result1);
         return result1;
      }
   }
   ...
   static EntityResolver access$000(EntityManager x0) {
      return x0.resolver;
   }
