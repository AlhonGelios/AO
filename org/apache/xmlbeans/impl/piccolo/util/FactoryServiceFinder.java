...
public class FactoryServiceFinder {
   ...
   public static Enumeration findServices(String name) throws IOException {
      return new FactoryServiceFinder.FactoryEnumeration(ClassLoader.getSystemClassLoader().getResources(name));
   }
         ...
         while(true) {
            try {
               if(this.enumValue.hasMoreElements()) {
                  BufferedReader e = new BufferedReader(new InputStreamReader(((URL)this.enumValue.nextElement()).openStream()));
                  this.next = e.readLine();
               } else {
                  this.next = null;
               }
