...
public final class SchemaTypeCodePrinter implements SchemaCodePrinter {
   ...
   private void emitSpecializedAccessors(SchemaType sType) throws IOException {
      if(sType.getSimpleVariety() == 1 && sType.getPrimitiveType().getBuiltinTypeCode() == 11) {
         int ctype = sType.getDecimalSize();
         int parentBits = sType.getBaseType().getDecimalSize();
         if(ctype != parentBits || sType.getBaseType().getFullJavaName() == null) {
            if(ctype == 1000000) {
      ...
      if(sType.getSimpleVariety() == 2) {
         ...
         SchemaType ctype1 = sType.getUnionCommonBaseType();
         if(ctype1 != null && ctype1.getSimpleVariety() != 2) {
            ;
         }

         this.emitSpecializedAccessors(ctype1);
      }
   ...
   void printStringEnumeration(SchemaType sType) throws IOException {
      ...
      if(!hasBase) {
         ...
         for(i = 0; i < entries.length; ++i) {
            enumValue = "INT_" + entries[i].getEnumName();
            int var11 = entries[i].getIntValue();
            this.emit("static final int " + enumValue + " = " + var11 + ";");
         }
   ...
   void printGetterImpls(String parentJavaName, SchemaProperty prop, QName qName, boolean isAttr, String propertyName, int javaType, String type, String xtype, boolean nillable, boolean optional, boolean several, boolean singleton, boolean isunion, String identifier, String setIdentifier) throws IOException {
      String propdesc = "\"" + qName.getLocalPart() + "\"" + (isAttr?" attribute":" element");
      boolean xmltype = javaType == 0;
      String jtargetType = !isunion && xmltype?xtype:"org.apache.xmlbeans.SimpleValue";
      if(singleton) {
         this.printJavaDoc((several?"Gets first ":"Gets the ") + propdesc);
   ...
   void printInnerTypeImpl(SchemaType sType, SchemaTypeSystem system, boolean isInner) throws IOException {
      ...
      if(!sType.isSimpleType()) {
         ...
         if(sType.getContentType() == 2) {
            SchemaType qNameMap = sType.getBaseType();

            ArrayList i;
            for(i = null; !qNameMap.isSimpleType() && !qNameMap.isBuiltinType(); qNameMap = qNameMap.getBaseType()) {
               SchemaProperty[] prop = qNameMap.getDerivedProperties();

               for(int name = 0; name < prop.length; ++name) {
                  if(!prop[name].isAttribute() || sType.getAttributeProperty(prop[name].getName()) == null) {
                     if(i == null) {
                        i = new ArrayList();
                     }
            ...
            if(i != null) {
               for(int var13 = 0; var13 < properties.length; ++var13) {
                  i.add(properties[var13]);
               }

               properties = (SchemaProperty[])((SchemaProperty[])i.toArray(new SchemaProperty[i.size()]));
            }
         } else {
            properties = this.getDerivedProperties(sType);
         }

         Map var11 = this.printStaticFields(properties);

         for(int var12 = 0; var12 < properties.length; ++var12) {
            SchemaProperty var14 = properties[var12];
            QName var15 = var14.getName();
            String xmlType = this.xmlTypeForProperty(var14);
            this.printGetterImpls(shortName, var14, var15, var14.isAttribute(), var14.getJavaPropertyName(), var14.getJavaTypeCode(), this.javaTypeForProperty(var14), xmlType, var14.hasNillable() != 0, var14.extendsJavaOption(), var14.extendsJavaArray(), var14.extendsJavaSingleton(), xmlTypeForPropertyIsUnion(var14), this.getIdentifier(var11, var15), this.getSetIdentifier(var11, var15));
            if(!var14.isReadOnly()) {
               this.printSetterImpls(var15, var14, var14.isAttribute(), var14.getJavaPropertyName(), var14.getJavaTypeCode(), this.javaTypeForProperty(var14), xmlType, var14.hasNillable() != 0, var14.extendsJavaOption(), var14.extendsJavaArray(), var14.extendsJavaSingleton(), xmlTypeForPropertyIsUnion(var14), this.getIdentifier(var11, var15), this.getSetIdentifier(var11, var15), sType);
            }
         }
      }
   ...
   private SchemaProperty[] getDerivedProperties(SchemaType sType) {
      ...
      if(name != null && name.equals(sType.getBaseType().getName())) {
         ...
         LinkedHashMap propsByName = new LinkedHashMap();
   ...
   void printInterfaceMethodDecl(InterfaceExtension.MethodSignature method) throws IOException {
      ...
      for(int exceptions = 0; exceptions < paramTypes.length; ++exceptions) {
         if(exceptions != 0) {
            decl.append(", ");
         }

         decl.append(paramTypes[exceptions]).append(" p").append(exceptions);
      }

      decl.append(")");
      String[] var6 = method.getExceptionTypes();

      for(int i = 0; i < var6.length; ++i) {
         decl.append((i == 0?" throws ":", ") + var6[i]);
      }
