...
public class SchemaTypeVisitorImpl implements TypeStoreVisitor {
   ...
   private SchemaTypeVisitorImpl.VisitorState[] _stack;
   private SchemaTypeVisitorImpl.VisitorState[] _rollback;
   int _stackSize;
   int _rollbackSize;
   private boolean _isValid;
   private SchemaParticle _matchedParticle;
   private SchemaTypeVisitorImpl.VisitorState _top;
   ...
   public void init(SchemaParticle part) {
      if(this._stack == null) {
         this._stack = this.expand((SchemaTypeVisitorImpl.VisitorState[])null);
      }

      if(this._rollback == null) {
         this._rollback = this.expand((SchemaTypeVisitorImpl.VisitorState[])null);
      }
   ...
   public SchemaTypeVisitorImpl.VisitorState[] expand(SchemaTypeVisitorImpl.VisitorState[] orig) {
      int newsize = orig == null?4:orig.length * 2;
      SchemaTypeVisitorImpl.VisitorState[] result = new SchemaTypeVisitorImpl.VisitorState[newsize];
      if(orig != null) {
         System.arraycopy(orig, 0, result, 0, orig.length);
      }

      for(int i = orig == null?0:orig.length; i < newsize; ++i) {
         result[i] = new SchemaTypeVisitorImpl.VisitorState((SchemaTypeVisitorImpl.NamelessClass279940181)null);
      }

      return result;
   }

   SchemaTypeVisitorImpl.VisitorState topRef() {
      return this._stack[this._stackSize - 1];
   }

   void saveCopy(SchemaTypeVisitorImpl.VisitorState ref) {
   ...
   void rollback() {
      while(this._rollbackSize > 0) {
         --this._rollbackSize;
         SchemaTypeVisitorImpl.VisitorState temp = this._stack[this._rollbackIndex];
         ...
         while(true) {
            ...
            while(true) {
               if(this._top._curCount >= this._top._curMax) {
                  ...
                  case 1:
                     skipped = this._top._processedChildCount;

                     for(int var8 = 0; var8 < this._top._childCount; ++var8) {
                        if(!this._top._seen[var8]) {
                           SchemaParticle candidate = this._top._curPart.getParticleChild(var8);
                           if(candidate.canStartWithElement(eltName)) {
                              ++this._top._processedChildCount;
                              this._top._seen[var8] = true;
                              this.push(candidate);
                              continue label124;
                           }
                     ...
                     break;
                  case 2:
                     for(skipped = 0; skipped < this._top._childCount; ++skipped) {
                        i = this._top._curPart.getParticleChild(skipped);
                        if(i.canStartWithElement(eltName)) {
                           ++this._top._curCount;
                           this.push(i);
                           continue label124;
                        }
                     ...
                     break;
                  case 3:
                     for(skipped = this._top._processedChildCount; skipped < this._top._childCount; ++skipped) {
                        i = this._top._curPart.getParticleChild(skipped);
                        if(i.canStartWithElement(eltName)) {
                           this._top._processedChildCount = skipped + 1;
                           this.push(i);
                           continue label124;
                        }
   ...
   static class NamelessClass279940181 {
   }
   ...
   private static class VisitorState {
      ...
      public void copy(SchemaTypeVisitorImpl.VisitorState orig) {
      ...
      VisitorState(SchemaTypeVisitorImpl.NamelessClass279940181 x0) {
         this();
      }
   }
}
