...
final class RangeToken extends Token implements Serializable {
   ...
   protected void addRange(int start, int end) {
      ...
      if(this.ranges == null) {
         ...
         if(this.ranges[var7 - 1] + 1 == r1) {
            this.ranges[var7 - 1] = r2;
            return;
         }

         int[] temp = new int[var7 + 2];
         System.arraycopy(this.ranges, 0, temp, 0, var7);
         this.ranges = temp;
         if(this.ranges[var7 - 1] >= r1) {
            this.setSorted(false);
         }
      ...
      if(this.ranges != null && this.ranges.length > 2) {
         if(!this.isCompacted()) {
            ...
            for(int target = 0; target < this.ranges.length; base += 2) {
               ...
               int result = this.ranges[base + 1];

               while(target < this.ranges.length && result + 1 >= this.ranges[target]) {
                  if(result + 1 == this.ranges[target]) {
                     if(DEBUG) {
                     ...
                     result = this.ranges[base + 1];
                     target += 2;
                  } else if(result >= this.ranges[target + 1]) {
                     if(DEBUG) {
                     ...
                     if(result >= this.ranges[target + 1]) {
            ...
            if(base != this.ranges.length) {
               int[] var5 = new int[base];
               System.arraycopy(this.ranges, 0, var5, 0, base);
               this.ranges = var5;
            }
   ...
   synchronized RangeToken getCaseInsensitiveToken() {
      if(this.icaseCache != null) {
         ...
         int i;
         for(int lowers = 0; lowers < this.ranges.length; lowers += 2) {
            for(i = this.ranges[lowers]; i <= this.ranges[lowers + 1]; ++i) {
               if(i > '\uffff') {
                  uppers.addRange(i, i);
               } else {
         ...
         RangeToken var6 = this.type == 4?Token.createRange():Token.createNRange();

         for(i = 0; i < uppers.ranges.length; i += 2) {
            for(int var7 = uppers.ranges[i]; var7 <= uppers.ranges[i + 1]; ++var7) {
               if(var7 > '\uffff') {
                  var6.addRange(var7, var7);
               } else {
                  char uch = Character.toUpperCase((char)var7);
                  var6.addRange(uch, uch);
               }
            }
         }

         var6.mergeRanges(uppers);
         var6.mergeRanges(this);
         var6.compactRanges();
         this.icaseCache = var6;
         return var6;
      }
   }
