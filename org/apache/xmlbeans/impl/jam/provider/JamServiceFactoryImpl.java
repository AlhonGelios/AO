...
public class JamServiceFactoryImpl extends JamServiceFactory {
   ...
   public JamClassLoader createSystemJamClassLoader() {
      ...
      try {
         JamService reallyUnexpected = this.createService(params);
         return reallyUnexpected.getClassLoader();
   ...
   protected JamClassBuilder createBuilder(JamServiceContext ctx) throws IOException {
      ...
      for(int barray = 0; barray < cls.length; ++barray) {
         if(log.isVerbose((Object)this)) {
            log.verbose("added classbuilder for classloader " + cls[barray].getClass());
         }

         builders.add(new ReflectClassBuilder(cls[barray]));
      }

      JamClassBuilder[] var9 = new JamClassBuilder[builders.size()];
      builders.toArray(var9);
      CompositeJamClassBuilder out = new CompositeJamClassBuilder(var9);
      out.init((ElementContext)ctx);
      if(log.isVerbose((Object)this)) {
         log.verbose("returning a composite of " + var9.length + " class builders.");
         MClass c = out.build("java.lang", "Object");
