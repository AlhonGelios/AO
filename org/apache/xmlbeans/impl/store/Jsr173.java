...
public class Jsr173 {
   ...
   public static Node nodeFromStream(XMLStreamReader xs) {
      if(!(xs instanceof Jsr173.Jsr173GateWay)) {
         return null;
      } else {
         Jsr173.Jsr173GateWay gw = (Jsr173.Jsr173GateWay)xs;
         Locale l = gw._l;
         if(l.noSync()) {
            l.enter();
   ...
   public static Node nodeFromStreamImpl(Jsr173.Jsr173GateWay gw) {
   ...
   public static XMLStreamReader newXmlStreamReader(Cur c, Object src, int off, int cch) {
      Jsr173.XMLStreamReaderForString xs = new Jsr173.XMLStreamReaderForString(c, src, off, cch);
      return (XMLStreamReader)(c._locale.noSync()?new Jsr173.UnsyncedJsr173(c._locale, xs):new Jsr173.SyncedJsr173(c._locale, xs));
   }
      ...
      if(k != 0 && k >= 0) {
         if(inner) {
            if(!c.hasAttrs() && !c.hasChildren()) {
               xs = new Jsr173.XMLStreamReaderForString(c, c.getFirstChars(), c._offSrc, c._cchSrc);
            } else {
               assert c.isContainer();

               xs = new Jsr173.XMLStreamReaderForNode(c, true);
            }
         } else {
            xs = new Jsr173.XMLStreamReaderForNode(c, false);
         }
      } else {
         xs = new Jsr173.XMLStreamReaderForString(c, c.getChars(-1), c._offSrc, c._cchSrc);
      }

      return (XMLStreamReader)(c._locale.noSync()?new Jsr173.UnsyncedJsr173(c._locale, (Jsr173.XMLStreamReaderBase)xs):new Jsr173.SyncedJsr173(c._locale, (Jsr173.XMLStreamReaderBase)xs));
   }
   ...
   private static final class XMLStreamReaderForNode extends Jsr173.XMLStreamReaderBase {
      ...
      static final boolean $assertionsDisabled = !Jsr173.class.desiredAssertionStatus();
   ...
   private static final class XMLStreamReaderForString extends Jsr173.XMLStreamReaderBase {
   ...
   private static final class UnsyncedJsr173 extends Jsr173.Jsr173GateWay implements XMLStreamReader, Location, NamespaceContext {

      public UnsyncedJsr173(Locale l, Jsr173.XMLStreamReaderBase xs) {
         super(l, xs);
      }
   ...
   private static final class SyncedJsr173 extends Jsr173.Jsr173GateWay implements XMLStreamReader, Location, NamespaceContext {
