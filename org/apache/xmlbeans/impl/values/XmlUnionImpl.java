...
public class XmlUnionImpl extends XmlObjectBase implements XmlAnySimpleType {
   ...
   protected boolean is_defaultable_ws(String v) {
      try {
         XmlAnySimpleType e = this._value;
         this.set_text(v);
         this._value = e;
         return false;
         ...
         try {
            for(boolean validate = true; validate || !this._validateOnSet(); validate = false) {
               for(int i = 0; i < members.length; ++i) {
                  try {
                     XmlAnySimpleType e = ((SchemaTypeImpl)members[i]).newValue(s, validate);
                     if(check(e, this._schemaType)) {
                        this._value = e;
                        return;
                     }
   ...
   public double getDoubleValue() {
      this.check_dated();
      return this._value == null?0.0D:((SimpleValue)this._value).getDoubleValue();
   }
   ...
   private static boolean logical_overlap(SchemaType type, int javacode) {
      ...
      if(javacode <= 46) {
         return type.getSimpleVariety() != 1?false:type.getPrimitiveType().getBuiltinTypeCode() == javacode;
      } else {
         switch(javacode) {
         case 47:
