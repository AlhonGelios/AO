...
public final class XsTypeConverter {
   ...
   public static float lexFloat(CharSequence cs) throws NumberFormatException {
      ...
      try {
         if(cs.length() > 0) {
            char e = cs.charAt(cs.length() - 1);
            if((e == 102 || e == 70) && cs.charAt(cs.length() - 2) != 78) {
               throw new NumberFormatException("Invalid char \'" + e + "\' in float.");
            }
         }
   ...
   public static String printFloat(float value) {
      return value == Float.POSITIVE_INFINITY?"INF":(value == Float.NEGATIVE_INFINITY?"-INF":(Float.isNaN(value)?"NaN":Float.toString(value)));
   }
   ...
   public static double lexDouble(CharSequence cs) throws NumberFormatException {
      ...
      try {
         if(cs.length() > 0) {
            char e = cs.charAt(cs.length() - 1);
            if(e == 100 || e == 68) {
               throw new NumberFormatException("Invalid char \'" + e + "\' in double.");
            }
         }
   ...
   public static String printDouble(double value) {
      return value == Double.POSITIVE_INFINITY?"INF":(value == Double.NEGATIVE_INFINITY?"-INF":(Double.isNaN(value)?"NaN":Double.toString(value)));
   }
   ...
   public static BigInteger lexInteger(CharSequence cs) throws NumberFormatException {
      if(cs.length() > 1 && cs.charAt(0) == 43 && cs.charAt(1) == 45) {
   ...
   public static boolean lexBoolean(CharSequence v) {
      ...
      default:
         break;
      case 4:
         if(116 == v.charAt(0) && 114 == v.charAt(1) && 117 == v.charAt(2) && 101 == v.charAt(3)) {
            return true;
         }
         break;
      case 5:
         if(102 == v.charAt(0) && 97 == v.charAt(1) && 108 == v.charAt(2) && 115 == v.charAt(3) && 101 == v.charAt(4)) {
            return false;
         }
      }

      String msg1 = "invalid boolean: " + v;
      throw new InvalidLexicalValueException(msg1);
   }
   ...
   public static QName lexQName(CharSequence charSeq, NamespaceContext nscontext) {
      ...
      for(firstcolon = 0; firstcolon < charSeq.length(); ++firstcolon) {
         if(charSeq.charAt(firstcolon) == 58) {
            hasFirstCollon = true;
            break;
         }
      }
   ...
   public static CharSequence lexAnyURI(CharSequence lexical_value) {
      ...
      for(int e = 0; e < URI_CHARS_TO_BE_REPLACED.length; ++e) {
         for(int i = 0; (i = s.indexOf(URI_CHARS_TO_BE_REPLACED[e], i)) >= 0; i += 3) {
            s.replace(i, i + 1, URI_CHARS_REPLACED_WITH[e]);
         }
      }
