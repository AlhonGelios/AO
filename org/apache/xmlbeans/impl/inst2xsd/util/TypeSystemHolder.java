...
public class TypeSystemHolder {
   ...
   public SchemaDocument[] getSchemaDocuments() {
      LinkedHashMap nsToSchemaDocs = new LinkedHashMap();
      Iterator schDocColl = this._globalElements.keySet().iterator();

      QName globalTypeName;
      String tns;
      SchemaDocument schDoc;
      while(schDocColl.hasNext()) {
         globalTypeName = (QName)schDocColl.next();
      ...
      schDocColl = this._globalAttributes.keySet().iterator();
      ...
      schDocColl = this._globalTypes.keySet().iterator();
      ...
      Collection schDocColl1 = nsToSchemaDocs.values();
      return (SchemaDocument[])((SchemaDocument[])schDocColl1.toArray(new SchemaDocument[schDocColl1.size()]));
   }
   ...
   private void fillUpEnumeration(Type type, org.apache.xmlbeans.impl.xb.xsdschema.Element parentSElement) {
      ...
      if(type.isQNameEnumeration()) {
         ...
         for(i = 0; i < type.getEnumerationValues().size(); ++i) {
            String var10 = (String)type.getEnumerationValues().get(i);
            restriction.addNewEnumeration().setValue(XmlString.Factory.newValue(var10));
         }
      }
   ...
   protected void fillUpLocalAttribute(Attribute att, org.apache.xmlbeans.impl.xb.xsdschema.Attribute sAttribute, String tns) {
      if(att.isRef()) {
         ...
         if(att.isOptional()) {
            sAttribute.setUse(org.apache.xmlbeans.impl.xb.xsdschema.Attribute.OPTIONAL);
         }
      }
   ...
   protected void fillUpContentForComplexType(Type type, ComplexType sComplexType, String tns) {
      if(type.getContentType() == 2) {
         SimpleContentDocument.SimpleContent explicitGroup = sComplexType.addNewSimpleContent();
         ...
         SimpleExtensionType i = explicitGroup.addNewExtension();
         i.setBase(type.getExtensionType().getName());
         this.fillUpAttributesInComplexTypesSimpleContent(type, i, tns);
      } else {
         ...
         if(type.getContentType() == 5) {
            ...
            var8 = sComplexType.addNewSequence();
         } else {
            ...
            var8 = sComplexType.addNewChoice();
            var8.setMaxOccurs("unbounded");
            var8.setMinOccurs(new BigInteger("0"));
         }

         for(int var9 = 0; var9 < type.getElements().size(); ++var9) {
            Element child = (Element)type.getElements().get(var9);

            assert !child.isGlobal();

            LocalElement childLocalElement = var8.addNewElement();
