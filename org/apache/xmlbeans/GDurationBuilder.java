...
public class GDurationBuilder implements GDurationSpecification, Serializable {
   ...
   private void _normalizeImpl(boolean adjustSign) {
      ...
      if(this._fs != null && (this._fs.signum() < 0 || this._fs.compareTo(GDate._one) >= 0)) {
         BigDecimal sign = this._fs.setScale(0, 3);
         this._fs = this._fs.subtract(sign);
         carry = (long)sign.intValue();
      }
      ...
      if(adjustSign && (this._D < 0 || this._CY < 0)) {
         int sign1 = this._D <= 0 && (this._CY < 0 || this._CY == 0 && this._M == 0)?-this._sign:this._getTotalSignSlowly();
         if(sign1 == 2) {
            sign1 = this._CY < 0?-this._sign:this._sign;
         }
   ...
   static boolean isValidDuration(GDurationSpecification spec) {
      return spec.getSign() != 1 && spec.getSign() != -1?false:spec.getYear() >= 0 && spec.getMonth() >= 0 && spec.getDay() >= 0 && spec.getHour() >= 0 && spec.getMinute() >= 0 && spec.getSecond() >= 0 && spec.getFraction().signum() >= 0;
   }
   ...
   static int compareDurations(GDurationSpecification d1, GDurationSpecification d2) {
      if(d1.getFraction().signum() == 0 && d2.getFraction().signum() == 0) {
         int diff = d1.getSign();
         int s2 = d2.getSign();
         long month1 = (long)diff * ((long)d1.getYear() * 12L + (long)d1.getMonth());
         long month2 = (long)s2 * ((long)d2.getYear() * 12L + (long)d2.getMonth());
         long sec1 = (long)diff * ((((long)d1.getDay() * 24L + (long)d1.getHour()) * 60L + (long)d1.getMinute()) * 60L + (long)d1.getSecond());
      ...
      GDurationBuilder diff1 = new GDurationBuilder(d1);
      diff1.subtractGDuration(d2);
      return diff1._getTotalSignSlowly();
   }
   ...
   public static String toPlainString(BigDecimal bd) {
      ...
      if(scale == 0) {
         ...
         boolean isNegative = intValStr.charAt(0) == 45;
   ...
   public static String stripTrailingZeros(String s) {
      ...
      int zeroIndex;
      for(zeroIndex = i; i >= 0 && s.charAt(i) == 48; --zeroIndex) {
         --i;
      }
