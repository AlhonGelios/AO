package org.openxmlformats.schemas.spreadsheetml.x2006.main.impl;

import javax.xml.namespace.QName;
import org.apache.xmlbeans.SchemaType;
import org.apache.xmlbeans.impl.values.XmlComplexContentImpl;
import org.openxmlformats.schemas.spreadsheetml.x2006.main.CTFill;
import org.openxmlformats.schemas.spreadsheetml.x2006.main.CTGradientFill;
import org.openxmlformats.schemas.spreadsheetml.x2006.main.CTPatternFill;

public class CTFillImpl extends XmlComplexContentImpl implements CTFill {

   private static final QName PATTERNFILL$0 = new QName("http://schemas.openxmlformats.org/spreadsheetml/2006/main", "patternFill");
   private static final QName GRADIENTFILL$2 = new QName("http://schemas.openxmlformats.org/spreadsheetml/2006/main", "gradientFill");


   public CTFillImpl(SchemaType var1) {
      super(var1);
   }

   public CTPatternFill getPatternFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         CTPatternFill var2 = null;
         var2 = (CTPatternFill)this.get_store().find_element_user(PATTERNFILL$0, 0);
         return var2 == null?null:var2;
      }
   }

   public boolean isSetPatternFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         return this.get_store().count_elements(PATTERNFILL$0) != 0;
      }
   }

   public void setPatternFill(CTPatternFill var1) {
      synchronized(this.monitor()) {
         this.check_orphaned();
         CTPatternFill var3 = null;
         var3 = (CTPatternFill)this.get_store().find_element_user(PATTERNFILL$0, 0);
         if(var3 == null) {
            var3 = (CTPatternFill)this.get_store().add_element_user(PATTERNFILL$0);
         }

         var3.set(var1);
      }
   }

   public CTPatternFill addNewPatternFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         CTPatternFill var2 = null;
         var2 = (CTPatternFill)this.get_store().add_element_user(PATTERNFILL$0);
         return var2;
      }
   }

   public void unsetPatternFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         this.get_store().remove_element(PATTERNFILL$0, 0);
      }
   }

   public CTGradientFill getGradientFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         CTGradientFill var2 = null;
         var2 = (CTGradientFill)this.get_store().find_element_user(GRADIENTFILL$2, 0);
         return var2 == null?null:var2;
      }
   }

   public boolean isSetGradientFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         return this.get_store().count_elements(GRADIENTFILL$2) != 0;
      }
   }

   public void setGradientFill(CTGradientFill var1) {
      synchronized(this.monitor()) {
         this.check_orphaned();
         CTGradientFill var3 = null;
         var3 = (CTGradientFill)this.get_store().find_element_user(GRADIENTFILL$2, 0);
         if(var3 == null) {
            var3 = (CTGradientFill)this.get_store().add_element_user(GRADIENTFILL$2);
         }

         var3.set(var1);
      }
   }

   public CTGradientFill addNewGradientFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         CTGradientFill var2 = null;
         var2 = (CTGradientFill)this.get_store().add_element_user(GRADIENTFILL$2);
         return var2;
      }
   }

   public void unsetGradientFill() {
      synchronized(this.monitor()) {
         this.check_orphaned();
         this.get_store().remove_element(GRADIENTFILL$2, 0);
      }
   }

}
