...
public class Repackager {
   ...
   public Repackager(String repackageSpecs) {
      ArrayList repackages = splitPath(repackageSpecs, ';');
      ...
      do {
         ...
         for(int spec = 1; spec < repackages.size(); ++spec) {
            String j = (String)repackages.get(spec - 1);
            from = (String)repackages.get(spec);
            if(j.indexOf(58) < from.indexOf(58)) {
               repackages.set(spec - 1, from);
               repackages.set(spec, j);
               i = true;
            }
         }
      } while(i);

      for(int var8 = 0; var8 < repackages.size(); ++var8) {
         String var9 = (String)repackages.get(var8);
         int var10 = var9.indexOf(58);
         if(var10 < 0 || var9.indexOf(58, var10 + 1) >= 0) {
            throw new RuntimeException("Illegal repackage specification: " + var9);
         }

         from = var9.substring(0, var10);
         String to = var9.substring(var10 + 1);
   ...
   void addPatterns(char sep, int off) {
      for(int i = 0; i < this._fromPackages.size(); ++i) {
         ...
         for(int toPackage = 0; toPackage < from.size(); ++toPackage) {
            if(toPackage > 0) {
               pattern = pattern + "\\" + sep;
            }

            pattern = pattern + from.get(toPackage);
         }
         ...
         this._toPackageNames[off + i] = var9;
      }
